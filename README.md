...
- CNY、USD、HKD、JPY、EUR、GBP 可按自选统计币种统一换算，也可关闭换算后分币种查看。
- 使用欧洲央行每日参考汇率并保存最后一次成功缓存；断网时不会把不同币种的数字直接相加。
...
- 可从系统照片选择器设置本机背景图片，并实时调整 0–40 级背景模糊。
...
续费记录、背景图片和通知均保存在本机。开启统一换算时，应用只会向欧洲央行请求公开参考汇率，不会上传续费项目、金额、备注、图片或账号信息。
